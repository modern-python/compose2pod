...
structural keys ([ADR-0008](0008-reject-structural-key-registry.md)), at
...
[ADR-0008](0008-reject-structural-key-registry.md) made for its own narrow
single-sourcing.

**Revisit trigger:** reopen if **three or more new strict-schema blocks arrive
that share one exact shape** — same pre-check, same `x-` policy, same
field-dispatch convention, same message frame — at which point a narrow helper
for *that one shape* (a
