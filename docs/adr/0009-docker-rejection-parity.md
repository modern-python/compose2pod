...
- **Docker accepts ⇒ compose2pod accepts, when podman can express it and it
  ...
  limitation** — a deferred piece of the subset, tracked as a GitHub issue —
  not a bug, and not a licence to refuse on taste.
...
- The subset write-up — "Parity on *refusal* is what the drop-in role demands;
  it is not parity for its own sake, and the package keeps its documented
  divergences elsewhere."
- The YAML-1.1-booleans change — "Refusing a file Docker runs is the one
...
Nothing checked either of them. Five consecutive changes each hand-found a
single divergence and fixed it, which is what a missing invariant looks like
from the outside. A measured sweep then found **113 more** across 672 probes.
...
**What the rule covers, and its one known residual.** The audit was explicit
that its 113 measured divergences were "a floor, not a total" — a single-key matrix cannot
...
sweeps could reach: the value grammars, the ignored and
...
add-host, stores, and pod options. So a
...
runs." It is catalogued as
modern-python/compose2pod#87 with a revisit trigger (a document-wide
pre-validation pass, independent of the closure, would close it),
...
recorded as a GitHub issue with a revisit trigger, and workable later — the live
instance is modern-python/compose2pod#86.
