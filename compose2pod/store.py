"""Podman secret store machinery shared by compose secrets and configs."""
...
def referenced_names(services: dict[str, Any], order: list[str], kind: StoreKind) -> list[str]:
...
def flags(svc: dict[str, Any], pod: str, kind: StoreKind) -> list[Token]:
...
def create_lines(compose: dict[str, Any], pod: str, project_dir: str, names: list[str], kind: StoreKind) -> list[str]:
...
def referenced_variables(compose: dict[str, Any], project_dir: str, names: list[str], kind: StoreKind) -> set[str]:
...
def validate_all(compose: dict[str, Any], kinds: tuple[StoreKind, ...]) -> None:
    """Validate every kind's top-level definitions and service references."""
    for kind in kinds:
        _validate_kind(compose, kind)


def all_flags(svc: dict[str, Any], pod: str, kinds: tuple[StoreKind, ...]) -> list[Token]:
    tokens: list[Token] = []
    for kind in kinds:
        tokens += flags(svc, pod, kind)
    return tokens


def all_teardown_names(services: dict[str, Any], order: list[str], pod: str, kinds: tuple[StoreKind, ...]) -> list[str]:
    names: list[str] = []
    for kind in kinds:
        names += [f"{pod}-{kind.prefix}{name}" for name in referenced_names(services, order, kind)]
    return names


def all_create_lines(
    compose: dict[str, Any],
    order: list[str],
    pod: str,
    project_dir: str,
    kinds: tuple[StoreKind, ...],
) -> list[str]:
    services = compose.get("services") or {}
    lines: list[str] = []
    for kind in kinds:
        lines += create_lines(compose, pod, project_dir, referenced_names(services, order, kind), kind)
    return lines
...
def all_referenced_variables(
    ...
    result: set[str] = set()
    for kind in kinds:
        result |= referenced_variables(compose, project_dir, referenced_names(services, order, kind), kind)
    return result
