...
through a form that key actually has.** The accepted forms are derived from the
key's own validator rather than from a hand-maintained list, so the two cannot
drift.
...
refuses standalone is also refused through `extends`. That is the property the
enumeration kept violating, and it now fails loudly rather than drifting. It is
verified to go red without the fix (on `ulimits`).
...
Two error messages improved: they now come from the key's validator
(`'cap_add' must be a list`) rather than the vaguer `cannot merge 'cap_add'
across incompatible forms`, matching what the same value yields standalone.
