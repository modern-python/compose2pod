...
the 8 keys that still have no `KeySpec`. A new `_pairs_to_mapping` in `keys.py`
...
`_concat_list(base, local)` normalizes both sides via the existing
list-or-str handling (moved from `extends._as_list`) and concatenates.
`_merge_map(base, local)` normalizes both sides via the new `_pairs_to_mapping`
(list-or-dict → dict, the inverse of `_key_value_pairs`, living beside it in
`keys.py`) and dict-merges, local winning. `ulimits`'s bespoke `KeySpec`
...
def _merge(base: dict[str, Any], local: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    for key, local_val in local.items():
        ...
        if key in base and spec is not None and spec.merge is not None:
            merged[key] = spec.merge(base[key], local_val)
...
`_MERGE_KEYS`/`_CONCAT_KEYS` shrink to the 8 keys with no `KeySpec`:
...
- Unifying the 8 structural keys' merge policy — deferred; would conflate this
